...
def main():
	...
	# Read data
	corpus_path = os.path.join(args.path, args.dataset, "Readers", model_name.reader+args.data_appendix+ '.pkl')
	...
	if args.save_final_results==1: # save the prediction results
		save_rec_results(data_dict['test'], runner, 10)
...
def save_rec_results(dataset, runner, topk):
	...
	save_appendix = model_name + "__" + dataset.corpus.dataset
	result_path = os.path.join(runner.log_path,save_appendix, 'rec-{}-{}.csv'.format(model_name,dataset.phase))
	...
	elif init_args.model_mode in ['TopK','', 'CPR']: # TopK Ranking task
...
if __name__ == '__main__':
	...
	if args.log_file == '':
		args.log_file = '../eval_log/{}/{}.txt'.format(init_args.model_name+init_args.model_mode, log_file_name)
	if args.model_path == '':
