...
class CPRReader(object):
    ...
    def _read_data(self):
        ...
        for key in ['train', 'dev', 'test']:
            ...
            # 唯一修改：增加了sample(frac=1, replace=False)来打乱数据，不对数据进行排序
            self.data_df[key] = pd.read_csv(os.path.join(self.prefix, self.dataset, key + '.csv'), sep=self.sep).sample(frac=1, replace=False).reset_index(drop=True)
